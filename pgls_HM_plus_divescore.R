...
tbllist_HM_plus_divescore<-list()
for (i in seq_along(pgls_models_list)){#change th 'Model' colume in this as appropriate
  tbllist_HM_plus_divescore[[i]]<-as.data.frame(summary(pgls_models_list[[i]])$'coefficients')
  tbllist_HM_plus_divescore[[i]]$Adj_Rsquared<-summary(pgls_models_list[[i]])$'adj.r.squared'[1]#rsquared
  tbllist_HM_plus_divescore[[i]]$Model<-modellist[i]#formula<_____________________CHECK FORMULA LIST HERE is correct
  tbllist_HM_plus_divescore[[i]]$Lambda<-summary(pgls_models_list[[i]])$'param'[[2]]#lambda
  tbllist_HM_plus_divescore[[i]]$Fstat<-summary(pgls_models_list[[i]])$fstatistic[1]
  tbllist_HM_plus_divescore[[i]]$Fstat_numdf<-summary(pgls_models_list[[i]])$fstatistic[2]
  tbllist_HM_plus_divescore[[i]]$Fstat_dendf<-summary(pgls_models_list[[i]])$fstatistic[3]
  tbllist_HM_plus_divescore[[i]]$AICc<-pgls_models_list[[i]]$aicc[1]
}

for(i in seq_along(tbllist_HM_plus_divescore)){
  tbllist_HM_plus_divescore[[i]]$Coefficients<-row.names(tbllist_HM_plus_divescore[[i]])
  tbllist_HM_plus_divescore[[i]]$Coefficients<-gsub('[[:digit:]]+', '', tbllist_HM_plus_divescore[[i]]$Coefficients)#regex to remove number automatically added during the loop
